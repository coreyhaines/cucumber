Feature: Step Extensions
  In order to maintain modularity within step definitions
  As a step definition editor
  I want to register step extensions for step definition arguments.
  ...
  Background:         
    ...
    And a file named "features/step_definitions/steps.rb" with:
      ...
      StepExtensions(:convert, MyExtensions)

      Then /^I should transform '(\d+)' to an Integer$/,
            :convert => :fixnum do |integer|
      end

      Then /^I should transform '(\w+)' to a Symbol$/,
            :convert => :symbol do |symbol|
